...
class RadarView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        radarClickButton?.addTarget(self, action: #selector(radarClickButton(_:)), forControlEvents: .TouchUpInside)
        self.addSubview(radarClickButton!)
        
        raderColor = UIColor.blueColor()
    }
    ...
    var isStart: Bool?{// 是否直接开始雷达  默认no
        willSet{
            if newValue == true{
                radarTimer = NSTimer.scheduledTimerWithTimeInterval(0.5, target: self, selector: #selector(drawCircle), userInfo: nil, repeats: true)
            }else{
